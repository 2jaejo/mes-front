...
const Main = ({ props={} }) => {
  ...
  useEffect(()=>{
    ...
    .then((res) => {
      ...
      setColumnDefs([
        ...
        { headerName: "매입가", field: "total_price", sortable: true, editable: false, align:"right",
          cellRendererSelector: (params) => {
            if (params.node.rowPinned) {
              return {
                component: ()=>{
      ...
      setColumnDefs2([
        ...
        { headerName: "품목코드", field: "raw_code", sortable: false, editable: false, align:"center"},
        ...
        { headerName: "비고", field: "comment", sortable: false, editable: (params) => !params.node.rowPinned, align:"left", flex:1}, 
      ]);
  ...
  const getData = (params) => {
    ...
    axiosInstance
    .post(`/api/getReceipt`, JSON.stringify(form))
    .then((res) => {
      setRowData(res.data);
    })
    .catch((error) => {
  ...
  const getData2 = (params) => {
    ...
    axiosInstance
    .post(`/api/getReceiptDet`, JSON.stringify(params))
    .then((res) => {
  ...
  const setData = (params) => {
    ...
    axiosInstance
      .post("api/setReceipt", JSON.stringify(params))
      .then((res) => {
  ...
  const setData2 = (params) => {
    ...
    axiosInstance
      .post("api/setReceiptDet", JSON.stringify(params))
      .then((res) => {
        getData(selectedRow.current);
      })
      .catch((error) => {
  ...
  const setData3= (params) => {
    ...
    if(sel_rows.length === 0){
      modalRef.current.open({ title:"알림", message:"입고항목을 선택하세요.", cancelText:"" }); 
      return;
    }
    ...
    axiosInstance
      .post("api/setReceiptClose", JSON.stringify(sel_rows))
      .then((res) => {
  ...
  const addData = (params) => {
    ...
    modalRef.current.open({
      ...
      onConfirm: (res) => {
        ...
        if(chk){
          modalRef2.current.open({ title:"알림", message:"입고수량을 입력하세요.", cancelText:"" });
          return ;
        }
        ...
        <Row className="">
          <Col className="overflow-auto">
            <Table bordered hover style={{ width: 'auto', tableLayout: 'auto' }} className="m-0">
              <tbody>
                <tr>
                  <th className="bg-light text-end align-middle">입고일자</th>
                  <td className="">
...
const ModalComponent = ({ form }) => {
  ...
  const getData2 = (params) => {
    ...
    modalRef.current.open({
      ...
      onConfirm: (res) => {
        ...
        modalFormChange({target:{name:'purchase_id', value:""}});
        setRowData([]);
  ...
  const getData4 = (params) => {
    ...
    modalRef.current.open({
      title: "발주 조회",
      content: <OrderComponent props={formRef4} style_props={{width:"80vw", height:"80vh"}} />,
      onCancel: ()=>{
        modalRef.current.close();
      },
      confirmText:"확인",
      ...
      onConfirm: (res) => {
        ...
        modalFormChange({target:{name:'client_name', value:row.sel_row.client_name}});

        const rows = row.sel_row2;
            ...
            <Table bordered style={{ width: 'auto', tableLayout: 'auto' }} className="m-0">
              <tbody>
                ...
                <tr>
                  ...
                  <th className="bg-light text-end align-middle">입고일</th>
                  <td className="">
                    <div className="d-flex gap-2">
                      <Form.Control 
                        type="date"
                        name="receipt_date"
                        value={modalForm.receipt_date}
                        onChange={modalFormChange}
                        size="sm" 
                        className="w-auto"
                        maxLength={50}
