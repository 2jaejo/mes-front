...
import SearchRawComponent from "components/SearchRawComponent";
...
const Main = ({ props={}, style_props={} }) => {
  ...
  useEffect(()=>{
    ...
    .then((res) => {
      ...
      setColumnDefs([
        ...
        { headerName: "거래처코드", field: "client_code", sortable: false, editable: false, align:"center", width:80 },
        { headerName: "거래처명", field: "client_name", sortable: false, editable: false, filter: "agTextColumnFilter", align:"left" , width:140},
        ...
        { headerName: "품목(수)", field: "raw_count", sortable: true, editable: false, align:"left", width:400},
        ...
        // { headerName: "주문", field: "order_chk", sortable: true, editable: false, align:"left", cellRenderer: ButtonRenderer},
        // { headerName: "마감", field: "close_chk", sortable: true, editable: false, align:"left", cellRenderer: ButtonRenderer},
      ...
      // 그리드 설정2
      setColumnDefs2([
        { headerName: "발주번호", field: "purchase_id", sortable: false, editable: false, align:"center", width:140},
        { headerName: "자재코드", field: "raw_code", sortable: false, editable: false, align:"center", width:140},
        ...
        { headerName: "납기일", field: "due_date", sortable: true, editable: (params) => !props.current && !params.node.rowPinned, filter: "agDateColumnFilter",  align:"center", cellDataType:'dateString'},
  ...
  const formR = useRef(form);

  // 검색창 입력필드 변경 저장
  const handleChange = (e) => {
    setForm(prev => ({ ...prev, [e.target.name]: e.target.value }));
    formR.current[e.target.name] = e.target.value;
  };


  const changeDate = (params) =>{
    console.log('changeDate');
    handleChange({target:{name:'start_date', value:dayjs().add(params, 'day').format('YYYY-MM-DD')}});
    handleChange({target:{name:'end_date', value:dayjs().format('YYYY-MM-DD')}});
  ...
  const addData = (params) => {
    ...
    modalRef.current.open({
      ...
      onConfirm: (res) => {
        ...
        if(!formRef.current.order_date){
          modalRef2.current.open({ title:"알림", message:"발주일을 입력하세요.", cancelText:"" });
          return;
        }
        ...
        const key = "quantity";
        const chk = arr.some(item => !item.hasOwnProperty(key) || item[key] === '' || item[key] === null || item[key] === undefined || item[key] <= 0);
        if(chk){
          modalRef2.current.open({ title:"알림", message:"발주수량을 입력하세요.", cancelText:"" });
          return ;
        }

        const key2 = "unit_price";
        const chk2 = arr.some(item => !item.hasOwnProperty(key2) || item[key2] === '' || item[key2] === null || item[key2] === undefined || item[key] <= 0);
        if(chk2){
          modalRef2.current.open({ title:"알림", message:"매입가를 입력하세요.", cancelText:"" });
          return ;
        }

        const key3 = "due_date";
        const chk3 = arr.some(item => !item.hasOwnProperty(key3) || item[key3] === '' || item[key3] === null || item[key3] === undefined);
        if(chk3){
          modalRef2.current.open({ title:"알림", message:"납기예정일을 입력하세요.", cancelText:"" });
          return ;
        }
      ...
      <div className="bg-light" >
        <Row className="">
          <Col className="overflow-auto">
            <Table bordered hover style={{ width: 'auto', tableLayout: 'auto', overflow:'auto' }} className="m-0">
              <tbody>
                <tr>
                  <th className="bg-light text-end align-middle">발주일자</th>
                  <td className="">
            ...
            <div className="mb-1 d-flex gap-2 justify-content-start align-items-center">
              ...
              {(Object.keys(props).length === 0) && 
                <>
                  ...
                  <Button size="sm" variant="secondary" onClick={print}>발주서 인쇄</Button>
                  <div className="print-only" ref={contentRef}>
                    <PrintOrderPage order={printOrder} items={printItems} />
                  </div>
...
const ModalComponent = ({ form }) => {
  ...
  const onGridReady = (params) => {
    ...
    params.api.addEventListener("cellValueChanged", (ev) => {
      ...
      if(col === "quantity" || col === "unit_price"){
        ...
        ev.node.setDataValue("total_price", supply);
  ...
  const delData = (params) => {
    ...
    if( sel_rows.length === 0){
      modalRef2.current.open({ title:"알림", message:"선택된 행이 없습니다.", cancelText:"" });
    }

    const sel_row_ids = sel_rows.map(item => item.raw_code);
    setRowData(prev => prev.filter(item => !sel_row_ids.includes(item.raw_code)));
            ...
            <Table bordered style={{ width: 'auto', tableLayout: 'auto' }} className="m-0">
              <tbody>
                <tr>
                  ...
                  <th className="bg-light text-end align-middle">발주일</th>
                  <td className="">
                    <div className="d-flex gap-2">
                      <Form.Control 
                        type="date"
