...
const Main = ({ props={} }) => {
  ...
  useEffect(()=>{
    ...
    .then((res) => {
      ...
      // 그리드 설정
      setColumnDefs([
        { headerName: "환입번호", field: "receipt_id", sortable: true, editable: false, filter: "agTextColumnFilter",  align:"center"},
        { headerName: "환입일자", field: "receipt_date", sortable: true, editable: false, filter: "agDateColumnFilter",  align:"center"},
      ...
      // 그리드 설정2
      setColumnDefs2([
        { headerName: "환입번호", field: "receipt_id", sortable: false, editable: false, align:"center"},
        ...
        { headerName: "환입수량", field: "received_qty", sortable: false, 
          align:"right", 
          editable: false , 
          cellRendererSelector: (params) => {
            if (params.node.rowPinned) {
  ...
  const addData = (params) => {
    ...
    modalRef.current.open({
      ...
      onConfirm: (res) => {
        ...
        axiosInstance
          .post(`/api/addReleaseReturn`, JSON.stringify(formRef.current))
          .then((res) => {
            getData();
            modalRef.current.close();
          })
        ...
        <Row className="">
          <Col className="">
            <Table bordered hover style={{ width: 'auto', tableLayout: 'auto' }} className="m-0">
              <tbody>
                <tr>
                  <th className="bg-light text-end align-middle">환입일자</th>
                  <td className="">
                  ...
                  <th className="bg-light text-end align-middle">환입번호</th>
                  <td className="">
                      <Form.Control 
                        type="text"
                        name="receipt_id"
        ...
        <Row  className="h-100">
          <Col className="h-100 d-flex flex-column" xs={12} md={12}>
            <div className="mb-1 d-flex gap-2 justify-content-start align-items-center">
              <span className="fw-bold">환입 목록</span>
...
const ModalComponent = ({ form }) => {
  ...
  useEffect(()=>{
    ...
    .then((res) => {
      ...
      setColumnDefs([
        ...
        { headerName: "기준단위", field: "base_unit", sortable: false, editable: false, align:"center"},
        // { headerName: "구매단위", field: "purchase_unit", sortable: false, editable: false, align:"center"},
        { headerName: "환입수량", field: "quantity", sortable: false, editable: true, align:"right", cellDataType: 'number',
          valueFormatter: (params) => moneyFormatter(params)
        },
  ...
  const getData4 = (params) => {
    ...
    modalRef.current.open({
      title: "발주 조회",
      content: <OrderComponent props={formRef4} />,
      onCancel: ()=>{
        modalRef.current.close();
      },
      confirmText:"확인",
      ...
      onConfirm: (res) => {
        ...
        modalFormChange({target:{name:'purchase_id', value:row.purchase_id}});
        modalFormChange({target:{name:'client_code', value:row.client_code}});
        modalFormChange({target:{name:'client_name', value:row.client_name}});
  ...
  const delData = (params) => {
    ...
    if (sel_rows.length === 0) {
      modalRef2.current.open({ title:"알림", message:"삭제할 항목을 선택하세요.", cancelText:"" });
      return;
    }

    // 선택된 항목의 ID만 추출
    const selectedIds = sel_rows.map(row => row.idx);

    // 상태에서 제거된 데이터로 재구성
    const updated = rowData.filter(row => !selectedIds.includes(row.idx));
